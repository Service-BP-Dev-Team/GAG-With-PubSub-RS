...
final class HashFunction {
	...
	static HashFunction getHashFunction() {
		...
		if (HashFunction.hashFunction == null) {
			try {
				HashFunction.hashFunction = new HashFunction(MessageDigest
						.getInstance("SHA-1"));
	 ...
	 *            Message digest for calculating hash values.
	 */
	private HashFunction(final MessageDigest digest1) {
		if (digest1 == null) {
	...
	private final ID createID(final byte[] testBytes) {
		/*
	 ...
	 */
	final ID createUniqueNodeID(final URL incomingURL) {
		if (incomingURL == null) {
			throw new IllegalArgumentException("URL must not be null!");
		}
		final String id = incomingURL.toString();
		final ID resultKey = createID(id.getBytes());
		return resultKey;
	}
	 ...
	 */
	final ID getHashKey(final Key entry) {
		...
		final byte[] testBytes = entry.getBytes();
