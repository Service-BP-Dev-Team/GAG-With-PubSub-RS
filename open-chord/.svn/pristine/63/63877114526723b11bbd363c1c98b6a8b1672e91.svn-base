...
public class URL implements Serializable {
	 ...
	 * Constant for URL parsing.
	 */
	private final static String DCOLON_SLASHES = URL.DCOLON + URL.SLASH
			+ URL.SLASH;
	 ...
	 *             pattern <code>protocol://host[:port]/path</code>, an unknown
	 *             protocol is specified, or port is negative.
	 */
	public URL(String urlString) throws MalformedURLException {
		...
		// parse protocol
		final int indexOfColonAndTwoSlashes = urlString
				.indexOf(URL.DCOLON_SLASHES);
		if (indexOfColonAndTwoSlashes < 0) {
			throw new MalformedURLException("Not a valid URL");
		}
		protocol = urlString.substring(0, indexOfColonAndTwoSlashes);
		urlString = urlString.substring(indexOfColonAndTwoSlashes + 3);

		// parse host and port
		int endOfHost = urlString.indexOf(URL.DCOLON);
		if (endOfHost > 0) {
			host = urlString.substring(0, endOfHost);
			urlString = urlString.substring(endOfHost + 1);
			final int endOfPort = urlString.indexOf(URL.SLASH);
			if (endOfPort < 0) {
				throw new MalformedURLException("Not a valid URL!");
			}
			/* initialise port */
			final int tmp_port = Integer.parseInt(urlString.substring(0,
					endOfPort));
			...
			endOfHost = urlString.indexOf(URL.SLASH);
			if (endOfHost < 0) {
				throw new MalformedURLException("Not a valid URL");
			}
			host = urlString.substring(0, endOfHost);
			urlString = urlString.substring(endOfHost + 1);
			if (protocol.equalsIgnoreCase(URL.KNOWN_PROTOCOLS
					.get(URL.SOCKET_PROTOCOL))) {
				port = URL.DEFAULT_PORTS[URL.SOCKET_PROTOCOL];
			} else if (protocol.equalsIgnoreCase(URL.KNOWN_PROTOCOLS
					.get(URL.RMI_PROTOCOL))) {
				port = URL.DEFAULT_PORTS[URL.RMI_PROTOCOL];
			} else {
				port = URL.DEFAULT_PORTS[URL.LOCAL_PROTOCOL];
			}
		}
		...
		for (int i = 0; i < URL.KNOWN_PROTOCOLS.size() && !protocolIsKnown; i++) {
			if (protocol.equals(URL.KNOWN_PROTOCOLS.get(i))) {
	 ...
	 * 
	 * @param obj
	 * @return <code>true</code> if provided <code>obj</code> is an instance of
	 *         <code>URL</code> and has the same attributes as this
	 *         <code>URL</code>.
	 */
	@Override
	...
	public final String toString() {
		if (urlString == null) {
			...
			builder.append(URL.DCOLON_SLASHES);
			builder.append(host);
			builder.append(URL.DCOLON);
			builder.append(port);
			builder.append(URL.SLASH);
			builder.append(path);
