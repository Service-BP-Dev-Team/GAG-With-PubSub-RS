 ...
 * realized in an pair of {@link Endpoint} and {@link Proxy}. This class has to
 * be extended by all Proxies that are used to provide a connection to a remote
 * node via the {@link Node} interface.
 * 
 * @author sven
 * @version 1.0.5
 */
public abstract class Proxy extends Node {
	...
	static {
		Proxy.logger.setParent(Activator.LOGGER);
	}

	/**
	 * Factory method to create a proxy to connect to the given {@link URL}. The
	 * protocol of url is used to determine the type of the proxy to create. The
	 ...
	 */
	public static Node createConnection(final URL sourceUrl,
			final URL destinationUrl) throws CommunicationException {
		...
		if (sourceUrl.equals(destinationUrl)) {
			Proxy.logger.severe("URLs are equal: this url= "
					+ sourceUrl.toString() + ", the other url= "
	 ...
	 * @param url
	 */
	protected Proxy(final URL url) {
		if (url == null) {
			throw new IllegalArgumentException("URL must not be null!");
		}
		nodeURL = url;
		Proxy.logger.info("Proxy with url " + url + " initialised.");
	}
