 ...
 * 
 *******************************************************************************/
// $Id: ConnectToOneCommand.java 2458 2008-10-06 06:42:03Z bboussema $
 ...
 */
public class ConnectToOneCommand extends ConnectToCommand {
	...
	public ConnectToOneCommand(final ComponentsManager cm,
	...
	@Override
	public void execute() {
		Activator.LOGGER.info(toString() + " connectToOneCommand.execute("
				+ typeSource + ", " + idSource + ", " + typeDestination + ", "
				+ idDestination + ", " + actions + ")");
		if (responseCount == 0) {
			if (possibleLocalDS != null
					&& possibleLocalDS.componentName.equals(idDestination)) {
			...
			if (possibleRemoteDS.size() > 0) {
				possibleRemoteDS.get(0).remoteDS.connectToOne(idSource,
						idDestination, typeDestination, dc, actions);
				cm.removeConnectToCommand(id);
				return;
			}
			if ((sc == null || !sc.equals("OFF"))
					&& possibleLocalDS != null
					&& possibleLocalDS.componentName.equals("TYPE@"
							+ typeDestination)) {
				possibleLocalDS.localDS.connectToOne(idSource, typeDestination,
						idDestination, actions);
				cm.removeConnectToCommand(id);
	...
	public void giveResponse(final String componentName, final LocalDS localDS,
		...
		Activator.LOGGER.info(toString() + " connectToOneCommand.giveResponse("
				+ componentName + ", " + localDS + ", " + remoteDS + ")");
